...
The administration password is rotated from the administration application instead, and needs no deployment: changing it signs out every other browser immediately and keeps the one making the change signed in. `HOME_GALLERY_ALLOW_ADMIN_UPLOADS=true` is the explicit compatibility control for browser uploads and should be enabled only when that feature is needed; it lets an administration session reach the otherwise ingestion-only routes, including while no password is set. Despite the name, that is every ingestion route rather than the upload route alone: with the flag on, an administration session may also register a Telegram contributor through `POST /api/telegram/contributors`. No decision is reachable that was not already, because registration cannot propose a status or override one an administrator made, and an administration session decides contributor status anyway. Changing it does need a restart, and the administration application shows or hides its upload control to match the next time the studio loads.
