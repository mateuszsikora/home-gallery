...
- Ingestion uses a rotatable bearer-token credential. Administration is protected by a password the administrator manages from the administration application, which is unset on a fresh installation; the two scopes are independent and neither credential works on the other's routes.
- The administration password is stored only as a salted scrypt hash, and changing or removing it invalidates every other administration session.
- Browser administration exchanges the password once for a bounded, short-lived, opaque HttpOnly session; cookie-authenticated mutations require explicit CSRF protection.
