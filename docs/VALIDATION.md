...
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Authorization      | Upload, media administration, and settings routes require a timing-safe bearer-token comparison. Health, playlist, and enabled normalized media are deliberately public. Tests cover missing and invalid credentials before mutations or upload reads.                                                                                                    |
...
| Browser boundary   | Production traffic is same-origin by default. CORS is disabled unless explicit origins are configured. nginx applies CSP, frame, MIME-sniffing, referrer, and permissions headers; direct API responses apply the relevant defensive headers too. The admin token is session-only and never enters URLs or build output.                                  |
...
The default LAN deployment uses HTTP. It is appropriate only on a trusted network. Use TLS or a trusted private overlay before exposing it across an untrusted network because the bearer token otherwise travels without transport encryption. Rate limiting, trusted-proxy handling, TLS automation, and separate administration/upload credentials are tracked as post-MVP defense in depth in [issue #21](https://github.com/mateuszsikora/home-gallery/issues/21).
...
Video, accounts and roles, multiple playlists, and real-time updates remain explicit post-MVP work. TLS termination and public-internet hardening are tracked in [issue #21](https://github.com/mateuszsikora/home-gallery/issues/21). They are not silently included in the current operational claims.
