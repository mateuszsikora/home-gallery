...
Focused tests additionally cover image signatures, corruption, EXIF orientation, decoded-pixel limits, multipart limits, concurrent file-count reservations, upload cleanup after database failure, deletion rollback after database failure, startup cleanup, pagination, authorization on every administrative route, bounded server-side session storage and expiry, HttpOnly/SameSite/Secure cookie policy, CORS with credentials, CSRF, token and cookie redaction, absence of browser token storage, bounded Telegram downloads, delete-on-success bot behavior, slideshow recovery, and every administration mutation.
