...
2. create an administration browser session, set an administration password and prove that a wrong one is refused, prove CSRF enforcement and logout, and upload a real PNG through the bearer-protected ingestion API;
3. read the item through the public gallery playlist and content proxy;
4. read the same administrative record through the authenticated administration proxy and reject the same request without a session;
5. change slideshow settings through the administration proxy and observe them through the public playlist;
6. stop the server with `SIGTERM`, require exit code `0`, restart it, verify persisted metadata, and prove that in-memory browser sessions were invalidated;
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Authorization      | Ingestion uses a rotatable bearer token and administration uses a password held only as a salted scrypt hash; both are compared in constant time. Ingestion credentials cannot manage media or settings, and an administration session cannot upload unless explicitly enabled. An installation with no password set is deliberately open to its network. Health, playlist, and enabled normalized media are deliberately public. Tests cover scope, ingestion rotation overlap, the password lifecycle, and rejection before request-body reads.                                                                                                                                                                                                            |
...
| Telegram ingestion | Contributor approval is checked before lookup or download, and the server independently refuses Telegram uploads from a contributor no administrator approved. Declared and streamed response sizes are capped. Every external operation has a timeout. Source messages are deleted only after the API confirms storage.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Browser boundary   | Production traffic is same-origin by default. CORS is disabled unless explicit origins are configured. nginx applies CSP, frame, MIME-sniffing, referrer, and permissions headers; direct API responses apply the relevant defensive headers too. The administration password is exchanged once for a bounded, short-lived, opaque HttpOnly/SameSite session and never enters browser storage, URLs, cookies, or build output; it is stored only as a salted scrypt hash. An installation with no password set is deliberately open to its network and says so in the administration application. Cookie mutations require a CSRF header; the optional Caddy profile adds Secure cookies, automatic TLS termination, and loopback-only direct HTTP bindings. |
...
| Containers         | Application containers are non-root, drop all capabilities, use read-only root filesystems, enable `no-new-privileges`, isolate their network and volume, and never address a Compose project they do not own.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
The default LAN deployment uses HTTP and is appropriate only on a trusted network. The defense-in-depth work in [issue #21](https://github.com/mateuszsikora/home-gallery/issues/21) adds a validated Caddy TLS profile, explicit trusted-proxy configuration, bounded invalid-authentication and upload attempts, and separate administration and ingestion credentials. An exhausted authentication limit is enforced before a password hash is derived, so a rejected client cannot make the server do that work. Public gallery playback does not consume either limiter. Deployments still need correct DNS, firewall policy, credential rotation, and monitoring; application controls do not turn a household service into a public multi-tenant platform.
...
- administrator-approved Telegram users can submit supported photos with attribution;
