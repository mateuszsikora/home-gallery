...
2. create an administration browser session, prove CSRF enforcement and logout, and upload a real PNG through the bearer-protected API;
...
| Browser boundary   | Production traffic is same-origin by default. CORS is disabled unless explicit origins are configured. nginx applies CSP, frame, MIME-sniffing, referrer, and permissions headers; direct API responses apply the relevant defensive headers too. The admin bearer is exchanged once for a bounded, short-lived, opaque HttpOnly/SameSite session and never enters browser storage, URLs, cookies, or build output. Cookie mutations require a CSRF header; the optional Caddy profile adds Secure cookies, automatic TLS termination, and loopback-only direct HTTP bindings. |
